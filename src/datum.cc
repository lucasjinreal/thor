...
LineSegment2d::LineSegment2d() { unit_direction_ = Vector2d(1, 0); }
...
LineSegment2d::LineSegment2d(const Vector2d &start, const Vector2d &end)
  ...
  length_ = hypot(dx, dy);
  unit_direction_ =
      (length_ <= kMathEpsilon ? Vector2d(0, 0)
...
double LineSegment2d::DistanceTo(const Vector2d &point,
                                 Vector2d *const nearest_pt) const {
  if (length_ <= kMathEpsilon) {
    *nearest_pt = start_;
    return point.DistanceTo(start_);
  }
...
double LineSegment2d::DistanceSquareTo(const Vector2d &point) const {
  if (length_ <= kMathEpsilon) {
...
double LineSegment2d::DistanceSquareTo(const Vector2d &point,
                                       Vector2d *const nearest_pt) const {
  if (length_ <= kMathEpsilon) {
    *nearest_pt = start_;
    return point.DistanceSquareTo(start_);
...
bool LineSegment2d::IsPointIn(const Vector2d &point) const {
  if (length_ <= kMathEpsilon) {
    return std::abs(point.x() - start_.x()) <= kMathEpsilon &&
        std::abs(point.y() - start_.y()) <= kMathEpsilon;
  }
  const double prod = thor::math::CrossProd(point, start_, end_);
...
double LineSegment2d::ProjectOntoUnit(const Vector2d &point) const {
  return unit_direction_.InnerProd(point - start_);
}

double LineSegment2d::ProductOntoUnit(const Vector2d &point) const {
...
double LineSegment2d::GetPerpendicularFoot(const Vector2d &point,
...
std::string LineSegment2d::DebugString() const {
  return "segment2d(start=" + start_.DebugString() + " end=" +
      end_.DebugString() + ")";
}

////////////////////////  AABox part ///////////////////////////
AABox2d::AABox2d(const Vector2d &center, const double length, const double width)
    : center_(center),
      length_(length),
      width_(width),
...
AABox2d::AABox2d(const Vector2d &one_corner, const Vector2d &opposite_corner)
    : AABox2d((one_corner + opposite_corner) / 2.0,
              std::abs(one_corner.x() - opposite_corner.x()),
              std::abs(one_corner.y() - opposite_corner.y())) {}

AABox2d::AABox2d(const std::vector<Vector2d> &points) {
  CHECK(!points.empty());
...
void AABox2d::GetAllCorners(std::vector<Vector2d> *const corners) const {
...
double AABox2d::DistanceTo(const Vector2d &point) const {
...
void AABox2d::Shift(const Vector2d &shift_vec) { center_ += shift_vec; }
...
void AABox2d::MergeFrom(const Vector2d &other_point) {
...
std::string AABox2d::DebugString() const {
  return "aabox2d(center=" + center_.DebugString() +
      " length=" + to_string(length_) + " width=" + to_string(width_) + ")";
}
...
Box2d::Box2d(const Vector2d &center, const double heading, const double length,
             const double width)
    : center_(center),
      length_(length),
      width_(width),
...
Box2d Box2d::CreateAABox(const Vector2d &one_corner,
...
void Box2d::GetAllCorners(std::vector<Vector2d> *const corners) const {
  if (corners == nullptr) {
    return;
  }
  *corners = corners_;
...
std::vector<thor::datum::Vector2d> Box2d::GetAllCorners() const {
  return corners_;
}
...
void Box2d::Shift(const Vector2d &shift_vec) {
  center_ += shift_vec;
  InitCorners();
}
...
std::string Box2d::DebugString() const {
  return "box2d(center=" + center_.DebugString() +
      " heading=" + to_string(heading_) + " length=" + to_string(length_) +
      " width=" +  to_string(width_) + ")";
}
...
Polygon2d::Polygon2d(std::vector<Vector2d> points) : points_(std::move(points)) {
  BuildFromPoints();
}

double Polygon2d::DistanceTo(const Vector2d &point) const {
...
double Polygon2d::DistanceSquareTo(const Vector2d &point) const {
  CHECK_GE(points_.size(), 3);
  if (IsPointIn(point)) {
    return 0.0;
  }
...
double Polygon2d::DistanceToBoundary(const Vector2d &point) const {
  double distance = std::numeric_limits<double>::infinity();
  for (int i = 0; i < num_points_; ++i) {
    distance = std::min(distance, line_segments_[i].DistanceTo(point));
  }
...
bool Polygon2d::IsPointOnBoundary(const Vector2d &point) const {
  CHECK_GE(points_.size(), 3);
  return std::any_of(
...
bool Polygon2d::IsPointIn(const Vector2d &point) const {
  CHECK_GE(points_.size(), 3);
  ...
  for (int i = 0; i < num_points_; ++i) {
    if ((points_[i].y() > point.y()) != (points_[j].y() > point.y())) {
      const double side = thor::math::CrossProd(point, points_[i], points_[j]);
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vector2d> &points,
  ...
  std::sort(sorted_indices.begin(), sorted_indices.end(),
            [&](const int idx1, const int idx2) {
              const Vector2d &pt1 = points[idx1];
              const Vector2d &pt2 = points[idx2];
...
bool Polygon2d::ComputeOverlap(const Polygon2d &other_polygon,
  ...
  std::vector<Vector2d> points = other_polygon.points();
  for (int i = 0; i < num_points_; ++i) {
...
bool Polygon2d::GetOverlap(const LineSegment2d &line_segment,
                           Vector2d *const first, Vector2d *const last) const {
  CHECK_GE(points_.size(), 3);
  CHECK_NOTNULL(first);
  CHECK_NOTNULL(last);
...
void Polygon2d::GetAllVertices(std::vector<Vector2d> *const vertices) const {
  if (vertices == nullptr) {
    return;
  }
  *vertices = points_;
...
std::vector<thor::datum::Vector2d> Polygon2d::GetAllVertices() const {
  return points_;
}
...
std::vector<LineSegment2d> Polygon2d::GetAllOverlaps(
  ...
  for (size_t i = 0; i + 1 < projections.size(); ++i) {
    ...
    const Vector2d reference_point =
        line_segment.start() +
...
void Polygon2d::ExtremePoints(const double heading, Vector2d *const first,
                              Vector2d *const last) const {
  CHECK_GE(points_.size(), 3);
  CHECK_NOTNULL(first);
  CHECK_NOTNULL(last);

  const Vector2d direction_vec = Vector2d::CreateUnitVec2d(heading);
...
Box2d Polygon2d::BoundingBoxWithHeading(const double heading) const {
  ...
  return Box2d(
      (x1 + x2) / 2.0 * direction_vec +
          (y1 + y2) / 2.0 * Vector2d(direction_vec.y(), -direction_vec.x()),
      heading, x2 - x1, y2 - y1);
}
  ...
  for (int i = 0; i < num_points_; ++i) {
    ...
    if (diff <= kMathEpsilon) {
      points.push_back(points_[i] +
          Vector2d::CreateUnitVec2d(start_angle) * distance);
    } else {
      ...
      for (int k = 0; k <= count; ++k) {
        ...
        points.push_back(points_[i] + Vector2d::CreateUnitVec2d(angle) * distance);
      }
    }
  }
...
std::string Polygon2d::DebugString() const {
  return "polygon2d(num_points=" + to_string(num_points_) + " points=(" +
      PrintDebugStringIter(points_) + ") is_convex: " + to_string(is_convex_)
