...
#define LOG_WARNING LogMessageThor(__FILE__, __FUNCTION__, __LINE__, "W")
#define LOG_FATAL LogMessageThorFatal(__FILE__, __FUNCTION__, __LINE__)
#define VLOG(level) \
  VLogMessageThor(__FILE__, __FUNCTION__, __LINE__, level).stream()
...
void gen_log(std::ostream& log_stream_, const char* file, const char* func,
             int lineno, const char* level, const int kMaxLen = 15);
...
#else
  LogMessageThor(const char* file, const char* func, int lineno,
                 const char* level = "I") {
...
class LogMessageThorFatal : public LogMessageThor {
 public:
#ifdef NDEBUG
  LogMessageThorFatal(...) : LogMessageThor() {}
  std::ostream& stream() { return log_stream_; }
#else
  LogMessageThorFatal(const char* file, const char* func, int lineno,
                      const char* level = "F")
...
class VLogMessageThor {
 public:
  ...
  std::ostream& stream() { return log_stream_; }
#else
  VLogMessageThor(const char* file, const char* func, int lineno,
